    ...
    <div className="space-y-0">
      ...
      <SectionBand>
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12">
          <Reveal className="lg:col-span-7">
            ...
            <h2 className="mt-4 text-3xl font-bold text-foreground lg:text-4xl">Our rich history</h2>
            <div className="mt-6 space-y-5 text-muted-foreground">
              {historyParagraphs.map((p, i) => (
                <p key={i} className={i === 0 ? 'text-lg leading-relaxed' : 'leading-relaxed'}>{p}</p>
              ))}
            </div>
            <Button className="mt-8 px-6" asChild>
          ...
          <Reveal className="lg:col-span-5" delay={120}>
            <div className="relative">
              <div className="site-arch-image relative aspect-[4/5] w-full overflow-hidden border border-border shadow-xl">
                <img
                  src={historyImage}
              ...
              <div className="absolute -bottom-6 left-6 right-6 border-l-4 border-l-gold bg-card p-5 shadow-lg">
      ...
      <SectionBand tone="muted" className="pt-24">
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 md:grid-cols-2">
          <Reveal>
            <article className="h-full border-t-4 border-t-gold bg-primary p-8 text-primary-foreground">
              <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/15 text-primary">
                <Eye className="h-6 w-6" />
              </span>
              <h3 className="mt-6 text-2xl font-bold text-primary-foreground">Our vision</h3>
              <p className="mt-3 leading-relaxed text-primary-foreground/70">{vision}</p>
            </article>
          </Reveal>
          <Reveal delay={100}>
            <article className="h-full border-t-4 border-t-gold bg-steel p-8">
          ...
          {target && (
            <Reveal delay={160} className="md:col-span-2">
              <article className="h-full border-l-4 border-l-gold bg-card p-8">
          ...
          {values.map((v, i) => {
            ...
            return (
              <Reveal key={v.title} delay={i * 90}>
                <article className="h-full border-t-4 border-t-gold bg-card p-7 shadow-sm transition-shadow duration-300 hover:shadow-lg">
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-6 sm:grid-cols-2">
          {leaders.map((l, i) => (
            <Reveal key={l.name} delay={i * 100}>
              <article className="group h-full overflow-hidden rounded-lg border border-border bg-card shadow-sm transition-all duration-300 hover:shadow-xl">
