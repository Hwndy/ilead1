...
serve(async (req) => {
  ...
  try {
    ...
    const portalUrl = `${(Deno.env.get('FRONTEND_URL') ?? 'https://ivintage.vercel.app').replace(/\/+$/, '')}/login?portal=true&role=parent`;
    ...
    const loginDomain = (typeof raw === 'string' ? raw : String(raw ?? '')) || 'students.ivintage.vercel.app';
        ...
        if (parentEmail) {
          ...
          if (!parentUser) {
            ...
            const { data: linkData } = await supabase.auth.admin.generateLink({
              type: 'recovery',
              email: parentEmail,
              options: { redirectTo: `${(Deno.env.get('FRONTEND_URL') ?? 'https://ivintage.vercel.app').replace(/\/+$/, '')}/reset-password` },
            });
